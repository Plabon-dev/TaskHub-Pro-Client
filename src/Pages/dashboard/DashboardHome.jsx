...
const DashboardHome = () => {
    ...
    useEffect(() => {
        axios.get(`https://task-hub-pro-server.vercel.app/taskList/${user?.email}`)
            .then(res => {
                setData(res?.data);
            });
    }, [user]);
    ...
    const handleDeleteItem = async (item) => {
        try {
            ...
            if (result.isConfirmed) {
                await axios.delete(`https://task-hub-pro-server.vercel.app/taskList/${item._id}`);
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
    ...
    const handleDrop = async (e, category) => {
        ...
        try {
            const updatedTask = await axios.patch(`https://task-hub-pro-server.vercel.app/updateCategory/${itemId}`, {
                category,
            });
        ...
        <div>
            ...
            <div className="m-4 md:m-16 flex flex-col lg:flex-row  justify-between gap-4 ">
                ...
                <div className="category w-full" onDrop={(e) => handleDrop(e, 'To-Do')} onDragOver={allowDrop}>
                    <h2 className="bg-gradient-to-r from-[#4c63cc] to-[#1c36ac] rounded-xl py-2 text-center w-full text-white font-bold ">To-Do</h2>
                ...
                <div className="category  w-full" onDrop={(e) => handleDrop(e, 'Ongoing')} onDragOver={allowDrop}>
                    <h2 className="bg-gradient-to-r from-[#4c63cc] to-[#1c36ac] rounded-xl py-2 text-center  w-full text-white font-bold">Ongoing</h2>
                ...
                <div className="category  w-full " onDrop={(e) => handleDrop(e, 'Completed')} onDragOver={allowDrop}>
                    <h2 className="bg-gradient-to-r from-[#4c63cc] to-[#1c36ac] rounded-xl py-2 text-center w-full text-white font-bold">Completed</h2>
